import websocket, threading, random, json, time, sys, os, requests
from colorama import Fore; from time import sleep
tokens = open("tokens.txt", "r").read().splitlines()

lc = Fore.LIGHTCYAN_EX; r = Fore.RESET
os.system('cls')
...
def scraper(guildid, channelid):
    with open("ids.txt", 'w+') as f: f.truncate(0)
    token = ""
    tokensinguild = []
    ...
    try:
        ...
        print("no valid tokens in guild")
        exit(0)
    members = scrape(token, f"{guildid}", f"{channelid}")
    for member in members:
        open("ids.txt",  "a+").write(str(member) + "\n")
